/**
 * CineLuxe - Edit Film Entry Controller
 * Handles loading existing film by URL query parameter (?id=...),
 * pre-populating fields, real-time preview synchronization,
 * updating film data in JSON storage, and film deletion.
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!filmId) {
    alert('No film ID specified for editing.');
    window.location.href = 'index.html';
    return;
  }

  // Header & Breadcrumb Elements
  const breadcrumbFilmId = document.getElementById('breadcrumbFilmId');
  const pageTitleText = document.getElementById('pageTitleText');
  const navBackDetail = document.getElementById('navBackDetail');
  ...
  const inputTagline = document.getElementById('editTagline');
  const inputYear = document.getElementById('editYear');
  const inputRating = document.getElementById('editRating');
  const inputDuration = document.getElementById('editDuration');
  const inputGenres = document.getElementById('editGenres');
  const inputDirector = document.getElementById('editDirector');
  const inputCast = document.getElementById('editCast');
  const inputPosterUrl = document.getElementById('editPosterUrl');
  const inputBackdropUrl = document.getElementById('editBackdropUrl');
  const inputViewUrl = document.getElementById('editViewUrl');
  const inputFeatured = document.getElementById('editFeatured');
  const inputSynopsis = document.getElementById('editSynopsis');

  const btnDeleteFilm = document.getElementById('btnDeleteFilm');
  ...
  // Preview Elements
  const previewArchiveId = document.getElementById('previewArchiveId');
  ...
  try {
    currentFilm = await window.movieService.getById(filmId);
    if (!currentFilm) {
      alert(`Film "${filmId}" not found in archive.`);
      window.location.href = 'index.html';
      return;
    }

    // 1. Populate Header & Navigation
    document.title = `Edit: ${currentFilm.title} — CineLuxe Archive`;
    breadcrumbFilmId.textContent = currentFilm.id.toUpperCase();
    pageTitleText.textContent = `Edit Film: ${currentFilm.title}`;
    navBackDetail.href = `detail.html?id=${encodeURIComponent(currentFilm.id)}`;
    btnCancelEdit.href = `detail.html?id=${encodeURIComponent(currentFilm.id)}`;
    previewArchiveId.textContent = `ID: ${currentFilm.id}`;

    // 2. Pre-fill Form Fields
    inputTitle.value = currentFilm.title || '';
    inputTagline.value = currentFilm.tagline || '';
    inputYear.value = currentFilm.year || 2024;
    inputRating.value = currentFilm.rating !== undefined ? currentFilm.rating : 8.0;
    inputDuration.value = currentFilm.duration || '';
    inputGenres.value = Array.isArray(currentFilm.genres) ? currentFilm.genres.join(', ') : '';
    inputDirector.value = currentFilm.director || '';
    inputCast.value = Array.isArray(currentFilm.cast) ? currentFilm.cast.join(', ') : (currentFilm.cast || '');
    inputPosterUrl.value = currentFilm.posterUrl || '';
    inputBackdropUrl.value = currentFilm.backdropUrl || '';
    inputViewUrl.value = currentFilm.viewUrl || '';
    inputFeatured.checked = Boolean(currentFilm.featured);
    inputSynopsis.value = currentFilm.synopsis || '';

    // Initialize Preview
    updateLivePreview();

  } catch (err) {
    console.error('Error loading film for editing:', err);
    alert('Failed to load film data.');
    window.location.href = 'index.html';
    return;
  }

  // 3. Live Real-Time Preview Synchronizer
  function updateLivePreview() {
    const titleVal = inputTitle.value.trim();
    previewTitle.textContent = titleVal || (currentFilm ? currentFilm.title : 'Movie Title');
  ...
  // 5. Delete Film Action
  btnDeleteFilm.addEventListener('click', async () => {
    const confirmDelete = confirm(`Are you sure you want to permanently delete "${currentFilm.title}" from the archive?`);
    if (!confirmDelete) return;

    try {
      await window.movieService.deleteMovie(currentFilm.id);
      showToast(`Film "${currentFilm.title}" deleted from archive.`, 'info');
      setTimeout(() => {
        window.location.href = 'index.html';
      }, 700);
    } catch (err) {
      console.error('Delete failed:', err);
      showToast('Failed to delete film from archive', 'error');
    }
  });

  // 6. Form Submission: Save Changes to JSON Archive
  form.addEventListener('submit', async (e) => {
    ...
    btnSubmitEdit.innerHTML = `
      ...
      <span>Updating Archive...</span>
    `;

    try {
      const updated = await window.movieService.updateMovie(currentFilm.id, {
        title,
        tagline,
        year,
        rating,
      ...
      showToast(`Updated "${updated.title}" in JSON Archive!`);

      // Redirect back to the film's detail dossier page
      setTimeout(() => {
      ...
      showToast('Failed to update film entry', 'error');
